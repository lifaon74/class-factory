...
export type TPipeFunction<GArg = any, GReturn = any> = (arg: GArg) => GReturn;

export type FollowsPipeConstraint<GFunctions extends TPipeFunction[], GFirstArg = any> =
  GFunctions extends []
    ? true
    : (
...
export type PipeFirstArg<GFunctions extends TPipeFunction[]> =
  GFunctions extends [infer GFirst, ...infer GRest]
    ? (
...
export type PipeLastReturn<GFunctions extends TPipeFunction[]> =
...
export type PipeReturn<GFunctions extends TPipeFunction[]> = (arg: PipeFirstArg<GFunctions>) => PipeLastReturn<GFunctions>;
...
export type PipeConstraint<GFunctions extends TPipeFunction[], GFirstArg = any> =
  [GFunctions] extends [[]]
    ? []
    : (
      [GFunctions] extends [[infer GFirst, ...infer GRest]]
...
export function pipe<GFunctions extends PipeConstraint<GFunctions>>(
  ...
  return (firstArg: PipeFirstArg<GFunctions>): PipeLastReturn<GFunctions> => {
    return (fns as TPipeFunction[]).reduce((arg: any, fnc: TPipeFunction) => fnc(arg), firstArg);
  };
}
...
export function compose(...fns: TPipeFunction[]) {
  return (firstArg: any): any => {
    return fns.reduceRight((arg: any, fnc: TPipeFunction) => fnc(arg), firstArg);
  }
}
